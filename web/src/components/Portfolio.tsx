  ...
  return (
    <div data-share-root="Pace">
      ...
      <div ref={chartRef} className="card" style={{ padding: '18px 18px 14px', marginBottom: 12 }}>
        ...
        {/* one 12-month row, drawn tall; the chart bleeds into the card padding
            so the twelve columns get every pixel of width */}
        <div style={{ margin: '0 -10px' }}>
          {k === 'occ'
            ? <OccPace months={months} tall />
            : k === 'rev'
              ? <BarPace months={months} field="rev" fieldStly="rev_stly" fieldFinal="rev_final" fmt={fmtAxis} fmtFull={v => euro(v)} tall />
              : <BarPace months={months} field="adr" fieldStly="adr_stly" fieldFinal="adr_final_ly" fmt={v => `€${Math.round(v)}`} fmtFull={v => `€${Math.round(v)}`} tall />}
        </div>
      </div>
      <div style={cardBox}>
        <style>{`@keyframes pwring{0%{background-position:0 0,0% 50%}50%{background-position:0 0,100% 50%}100%{background-position:0 0,0% 50%}}`}</style>
